...
/* Check if language options ar declared as environment variables and set default values if not explicitly declared */
const defaultSource = process.env.JA_GTC_SOURCE || 'en';
const defaultTarget = process.env.JA_GTC_TARGET || 'es';

/* Supported languages: https://cloud.google.com/translate/v2/translate-reference#supported_languages */

/* Creates the URL to get the translation from Google's server
...
program
  ...
  .on('--help', () => {
    ...
    console.log("\t$ pbpaste | translate # when text stored in Mac clipboard");
  })
  .parse(process.argv);

/* Shows all languages (code and name) */
if (program.list) {
